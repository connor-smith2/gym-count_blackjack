...
def draw_hand(np_random):
    if deck.length() < 2:
        ...
        valo = draw_card(np_random)
        deck.remove(valo)
        valt = draw_card(np_random)
        deck.remove(valt)
        if int(valo) == 1 or int(valo) == 10:
            count -= 1
        if int(valo) < 7:
            count += 1
        if int(valt) == 1 or int(valt) == 10:
            count -= 1
        if int(valt) < 7:
            count += 1
        return [valo, valt]
...
class BlackjackEnv(gym.Env):
    ...
    def _get_obs(self):
        return (sum_hand(self.player), self.dealer[0], usable_ace(self.player), countn)
